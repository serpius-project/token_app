...
var CONTRACT_NAME = "token.serpius_v0.testnet" || 'token.serpius_v0.testnet'; //const CONTRACT_NAME = process.env.CONTRACT_NAME || 'token.serpius.near'
  ...
  switch (env) {
    ...
    case 'local':
      return {
        ...
        keyPath: "".concat("/Users/mmendozaj", "/.near/validator_key.json"),
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 27:
            ...
            for (i = 0; i < window.distro.length; i++) {
              window.distro_real[i] = window.distro[i] * window.prices[i];
              distro_real_norm += window.distro_real[i];
            }

            for (_i = 0; _i < window.distro.length; _i++) {
              window.distro_real[_i] = Math.round(1000 * window.distro_real[_i] / distro_real_norm);
            }
            ...
            for (_i3 = 0; _i3 < 4; _i3++) {
              total_value += window.distro[_i3] * window.prices[_i3];
            }
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "65258" + '/');
